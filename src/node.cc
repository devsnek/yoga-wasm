...
static YGSize globalMeasureFunc(YGNodeRef nodeRef,
                                float width,
...
// static
Node* Node::createDefault(void) {
  return new Node(nullptr);
}

// static
Node* Node::createWithConfig(Config* config) {
...
// static
Node* Node::fromYGNode(YGNodeRef nodeRef) {
  return reinterpret_cast<Node*>(YGNodeGetContext(nodeRef));
}

Node::Node(Config* config)
    : m_node(config != nullptr ? YGNodeNewWithConfig(config->m_config)
                               : YGNodeNew()),
...
void Node::setFlexDirection(int flexDirection) {
  YGNodeStyleSetFlexDirection(m_node,
...
YGSize Node::callMeasureFunc(double width,
                             YGMeasureMode widthMode,
                             double height,
...
void Node::calculateLayout(double width, double height, int direction) {
  YGNodeCalculateLayout(m_node, width, height,
